...
import com.lyx.goods.entity.vo.RecommendVo;
...
public class RecommendServiceImpl extends ServiceImpl<RecommendMapper, Recommend> implements RecommendService {
    ...
    public List<RecommendVo> getRecommend() {
        ...
        wrapper.eq(Goods::getIsOnSell,1);
        List<Goods> goods = goodsService.list(wrapper);
        List<Recommend> recommends = list();
        List<RecommendVo> recommendVos = goods.stream().filter(goods1 -> {
            for (Recommend recommend : recommends) {
                return goods1.getId() != recommend.getGoodsId();
            }
            return false;
        }).map(goods1 -> {
            RecommendVo recommendVo = new RecommendVo();
            BeanUtils.copyProperties(goods1, recommendVo);
            return recommendVo;
