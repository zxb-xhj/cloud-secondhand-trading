<template>
  <div id="detail" v-loading="listLoading">
    ...
    <a style="margin-left: 3.5vw;float: left;margin-top: -3.7rem;" class="el-dropdown-link"
    ...
    <el-row>
      ...
      <el-col :span="8">
        <div class="grid-content bg-purple">
          <el-col>
            <div style="text-align: center;margin-bottom: 5%;font-size: 3.3vh;" class="grid-content bg-purple title">{{
              title }}</div>
          </el-col>
          <img :src="commodityImage" style="width: 23vw;height: 47vh;margin-left: 6vw" alt="">
          <div style="margin-top: 3.2vh;margin-left: 5vw;">
            <div id="info" class="info"
              style="height: 10vh;width: 25vw;text-align: center;border-radius: 10px;margin-left: 0.3vw;border: #ffd69d solid 1px;">
              ...
              <div style="width: 25vw;height: 8vh;border-radius: 0.5rem;margin-left: 2vw;" @click="Have()">
                ...
                <div style="float: left;margin-top: 0.3vh;font-size: 2.5vh;width: 9vw;">
                  ...
                  <div
                    style="clear: left;font-size: 1.3vh;background-color:#000000;background-color:rgba(255, 115, 0, 0.51);border-radius: 10px;height: 3vh;width: 4vw;">
      ...
      <el-col :span="16">
        <div class="detail-box">
          <el-row>
            <el-col :span="24">
              <p style="font-size: 2.6vh;float: left;">商品简介</p>
              <div style="float: left;border: 2px solid #f9a837;border-radius: 7px;margin-left: 1.5vw;">
                <div
                  style="height: 2.9vh;text-align: center;font-size: 0.9vw;color: #f59712;margin-left: 0.3vw;margin-right: 0.3vw;">
                  <div v-if="deliveryMethod == 2 && freight == 0">包邮</div>
                  <div v-if="deliveryMethod == 1">自提</div>
                  <div v-if="deliveryMethod == 0 && freight == 0">包邮或自提</div>
                  <div v-if="deliveryMethod != 0 && freight != 0">不包邮</div>
                  <div v-if="deliveryMethod == 0 && freight != 0">不包邮或自提</div>
                </div>
              </div>
              <div style="float: right;width: 2vw;height: 2vh;margin-right: 3vw;">
                <img class="el-dropdown-link" style="float: right;width: 1.8vw;height: 3.2vh;" v-if="ai"
                  @click="collection(1)" src="../../../public/a1.png">
                <img class="el-dropdown-link" style="float: right;width: 1.8vw;height: 3.2vh;" v-else
                  @click="collection(2)" src="../../../public/a2.png">
              </div>
            </el-col>
            <div style="height: 22vh;">
              ...
              <el-col :span="24">
                <span v-for="img in images" :key="img.id">
                  <el-image style="width: 10.5vw;height: 22vh;margin-left: 1vw;" alt="无法显示" :src="img.url"
            ...
            <div style="width: 100%;font-size: 2.2vh;">
              <div v-if="schoolName != null" style="float: left;font-size: 2.5vh;">自提地址：<p style="float: right;">{{ schoolName }}</p></div>
              <div style="float: right;">库存:{{ save }}</div>
              <div style="float: right;margin-right: 5rem;">浏览量:{{ viewCount }}</div>
              <div style="float: right;margin-right: 5rem;">运费:{{ freight }}</div>
    ...
    <div style="clear: left;width: 100%;margin-top: 5vh;" v-loading="listLoading1">
      ...
      <div v-if="messages">
        <div v-for="(message, index) in messages" :key="index" style="width: 100%;">
          ...
          <div style="margin-left: 15vw;width: 70%;clear: left;margin-top: 1.5vh;">
            ...
            <div style="float: left;">
              <div style="font-size: 1.7vh;color: #787878;margin-top: 0.5vh;">{{ message.memberName }} · <span
                  style="font-size: 1.5vh;">{{ message.createTime }}</span></div>
              <div style="font-size: 2vh;"><span v-if="message.parentName" style="font-size: 1.8vh;color: #d1d1d1;">回复:{{
                message.parentName }}({{ message.parentContent }})</span> {{ message.content }}</div>
              <div @click="deleteMessage(message.id, index)" v-if="message.memberName == name" class="el-dropdown-link"
                style="font-size: 1.5vh;margin-top: 0.5vh;">删除</div>
              <div @click="huifu(message.id, message.memberName, message.content)" v-if="message.memberName != name"
                class="el-dropdown-link" style="font-size: 1.5vh;margin-top: 0.5vh;">回复</div>
      ...
      <div v-loading="listLoading1" v-if="messages.length == 0"
        style="font-size: 2vh;text-align: center;margin-top: 8vh;margin-bottom: 8vh;height: 10vh;">暂无评论</div>
    </div>
...
export default {
  ...
  watch: {
    ...
    // gid: function (val) {
    //   this.getGoodById(this.$route.params.gid)
    // }
  },
  methods: {
    // 收藏
    collection(state) {
      this.$http.get("/api/cloud-goods/collect/collection",
        { params: { memberId: sessionStorage.getItem("user_id"), goodsId: this.gid, state: state } }).then(res => {
          if (state == 1) {
            this.ai = false
    ...
    getCollection() {
      this.$http.get("/api/cloud-goods/collect/getCollection",
        { params: { memberId: sessionStorage.getItem("user_id"), goodsId: this.gid } }).then(res => {
          if (res.data == 1) {
            this.ai = false
          } else {
            this.ai = true
      ...
      if (sessionStorage.getItem("user_id") == null) {
        ...
        if (message != null && message != '') {
          ...
          this.$http.post("/api/cloud-message/commentReply/save",
            ...
            this.messages.push({
              id: res.data.data.id, memberName: name, content: res.data.data.content,
    ...
    // 添加浏览记录
    getFootprint(id) {
      this.$http.get("/api/cloud-goods/footprint/addFootprint", { params: { goodsId: id, memberId: sessionStorage.getItem('user_id') } }).then(res => {
    ...
    getGoodById(id) {
      ...
      this.$http.get("/api/cloud-goods/goods/" + id).then(res => {
        if (res.data.data != null) {
          ...
          that.deliveryMethod = res.data.data.deliveryMethod
          that.isCoeducation = res.data.data.isCoeducation
          that.schoolName = res.data.data.schoolName
