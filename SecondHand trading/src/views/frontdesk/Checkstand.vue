          ...
          <div style="margin-left: 4%;width: 100%;">
            ...
            <div style="float: left;">
              ...
              <div style="margin-top: 8%;">
                <!-- <router-link :to="{ path: '/djs' }"> -->
                  请您在 <span style="color: #da2525;">
                    <djs :time="time" :endTime="time"  :callback="getOrderId()"></djs>
                  </span> 内完成支付，否则订单将自动取消。
                <!-- </router-link> -->
              </div>
              <div style="height: 20px;"></div>
              <div v-if="display">
                <div style="font-size: 13px;color: #414141;">订单号：<span style="color: #e01515;">{{ id }}</span></div>
                <div style="font-size: 13px;color: #414141;">收货信息：<span style="font-size: 12px;color: #414141;">{{
                  addr.consignee }} </span><span style="font-size: 12px;color: #414141;"> {{ addr.mobile }}</span></div>
                <div style="font-size: 13px;color: #414141;">商品名称：<span style="font-size: 12px;color: #414141;">{{
                  goods.name }}</span></div>
                <div style="font-size: 13px;color: #414141;">购买时间：<span style="font-size: 12px;color: #414141;">{{
                  addr.createTime }}</span></div>
              </div>

            </div>

            <div style="float: right;margin-right: 8%;">
              <span style="margin-left: 15px;">订单金额：</span><span style="color: #e01515;font-size: 18px;">￥{{ goods.price
              }}</span>
...
import djs from './userinfo/djs.vue';
export default {
  components: { djs },
  name: "nowhome",
  beforeUpdate() {
  ...
  methods: {
    getTime1(){
      this.$http.get("/api/cloud-order/order/time/" + this.id ).then(res => {
