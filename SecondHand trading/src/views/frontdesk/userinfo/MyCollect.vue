        ...
        <div v-for="item in list" :key="item.goodsId">
            <div class="photo-list" style="float: left;background-color: rgb(249, 249, 249);border-radius: 25px;border: 1px;width: 14.5vw;margin-right: 1.5vw;height: 40vh;
                ...
                <div style="z-index:0;position: absolute;">
                    ...
                    white-space: nowrap;overflow: hidden;">{{ item.name }}</div>
                    <div style="height: 5px;"></div>
                    <router-link :to="{ path: '/detail/' + [[item.goodsId]] }">
                        <img :src="item.defaultImg" alt=""
                            style="height: 27vh;width: 11vw;margin-left: 1.8vw;border-radius: 5px;">
                    </router-link>
                ...
                border: 1px;width: 14.5vw;height: 40vh;" @click="deleteId(item.goodsId)">
                    <img class="img" style="z-index:1;position: absolute;width: 4.5vw;height: 10vh;margin-top: 13vh;margin-left: 5vw;"
...
export default {
    name: "MyCollect",
    props: {list:{Array}, display:{type:Boolean},getCollectList:{type:Function}},
    data: {
        return() {
    ...
    methods:{
      ...
      this.$confirm(`确定删除吗？`, '是否继续?', '提示', {
        ...
        this.$http.get("/api/cloud-goods/collect/collection", { 
          params: {  memberId: sessionStorage.getItem('user_id'),goodsId: id,state: 2 }}).then(res => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })// 获取商品列表
