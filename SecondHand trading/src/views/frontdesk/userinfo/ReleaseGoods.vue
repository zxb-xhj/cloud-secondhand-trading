  ...
  <div>
    ...
    <template>
      ...
      <el-table v-loading="listLoading" :row-style="{ height: '45px' }" :cell-style="{ padding: '0px' }" :data="List"
        ...
        <el-table-column align="center" prop="created_at" label="操作">
          <template v-slot="scope">
            <el-button v-if="scope.row.audit.state != 2" type="primary" icon="el-icon-edit" size="mini"
              @click="ReleaseMember(scope.row.audit.goodsId)">详情</el-button>
            ...
            <el-button @click="deleteid(scope.row.audit.goodsId, scope.row.isOnSell)"
              v-if="scope.row.audit.state == 1 && scope.row.isOnSell == 1" type="danger" icon="el-icon-delete"
    ...
    <el-dialog :title="goodsFormTitle" :visible.sync="goodsFormShow">
      <el-form :rules="rules" :model="goodsForm" label-width="80px">
        ...
        <el-form-item prop="deliveryMethod" label="收货方式">
          <el-radio-group v-model="goodsForm.deliveryMethod">
            <el-radio :label="1">自提</el-radio>
            <el-radio :label="2">快递</el-radio>
            <el-radio :label="0">自提/快递</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item prop="schoolName" label="自提学校"
          v-if="goodsForm.deliveryMethod == 1 || goodsForm.deliveryMethod == 0">
          <el-cascader size="medium" :props="sprops" v-model="goodsForm.schools" :options="schoolList" filterable></el-cascader>
        </el-form-item>
        <el-form-item prop="addr" label="详细地址"
          v-if="goodsForm.deliveryMethod == 1 || goodsForm.deliveryMethod == 0">
          <el-input v-model="goodsForm.addr"></el-input>
        </el-form-item>
        <el-form-item prop="freight" label="运费" v-if="goodsForm.deliveryMethod == 2 || goodsForm.deliveryMethod == 0">
          <el-input v-model="goodsForm.freight" />
        </el-form-item>
        ...
        <el-form-item prop="defaultImg" label="展示图">
          <el-upload :data="dataObj" action="http://cloud-zxb.oss-cn-guangzhou.aliyuncs.com" multiple
            ...
            <img style="width: 9.3rem;height: 9.3rem;" v-if="defaultImg!=''" :src="defaultImg"
              class="avatar">
...
export default {
  ...
  methods: {
    ...
    deleteid(goodsId, isOnSell) {
      ...
      this.state = 1
      this.$http.patch("/api/cloud-goods/goods/" + isOnSell + "/" + goodsId + "/" + sessionStorage.getItem("user_id")).then(res => {
        this.$message({
          type: 'success',
          message: '操作成功!'
        })
    ...
    handleSave() {
      ...
      this.goodsForm.schoolName = ''
      if (this.goodsForm.deliveryMethod == 1) {
        this.goodsForm.freight = 0
      }
      if (this.goodsForm.deliveryMethod == 0 || this.goodsForm.deliveryMethod == 1) {
        this.goodsForm.schoolName = this.goodsForm.schools[2]
      }
      console.log(this.goodsForm)
      // 保存商品信息
      // this.$refs['goodsForm'].validate((valid) => {  //开启校验
      //   if (valid) {
      if (this.state == 0) {
    ...
    handleUploadSuccess(res, file) {
      ...
      this.goodsForm.images = this.images
    },
    ...
    handleDefaultImgUploadSuccess(res, file) {
      this.defaultImg = this.dataObj.host + '/' + this.dataObj.key;
      this.goodsForm.defaultImg = this.defaultImg
