<template>
  <div>
    <template>
      <el-table v-loading="listLoading" :row-style="{ height: '45px' }" :cell-style="{ padding: '0px' }" :data="orderList"
        ...
        <el-table-column label="订单状态" align="center" width="100">
          <template v-slot="scope">
            <el-tag type="primary" effect="dark" v-if="scope.row.state == 1">待发货</el-tag>
            ...
            <el-tooltip class="item" effect="dark" :content="scope.row.mark" placement="top">
              <el-tag type="success" effect="dark" v-if="scope.row.state == 2">已发货</el-tag>
            </el-tooltip>
            <el-tag type="success" effect="dark" v-if="scope.row.state == 3">已完成</el-tag>
            <el-tag type="info" effect="dark" v-if="scope.row.state == 4">已取消</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="created_at" label="操作">
          <template v-slot="scope">
            ...
            <!-- <el-button v-if="scope.row.state == 0 || scope.row.state == null" type="primary" icon="el-icon-edit" size="mini"
              @click="payment(scope.row.orderSn)">付款</el-button> -->
            <!-- </router-link> -->

            <el-button type="primary" icon="el-icon-edit" size="mini"
              @click="getOrder(scope.row.orderSn, scope.row.id)">详情</el-button>
            <el-button v-if="scope.row.state == 1 || scope.row.state == 0 || scope.row.state == null" type="danger"
              icon="el-icon-delete" size="mini" @click="deleteById(scope.row.orderSn, scope.row.id,scope.row.goodsId)">取消</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async deleteById(orderSn,id, goodsId) {
