    ...
    <div>
            ...
            <div style="overflow: auto;height: 77.5vh;">
                <div v-for="(name,index) in userList" :key="name.id">
                    <div v-bind:ref="name.id"  v-bind:id="name.id+'id'" class="messagediv" style="width: 18.5vw;height: 8vh;border-radius: 1.3vh;margin-left: 0.7vw;" @click="getStyMessage(name)">
                        ...
                        <div v-if="name.readCount>0&&name.readCount<10" style="float: left;margin-top: 0.3vh;font-size: 2.2vh;margin-left: -1.4vw;background-color: red;border-radius: 50%;width: 1.4vw;">
                            <div style="font-size: 1.5vh;color: white;
                            margin-top: 0.45vh;margin-bottom: 0.45vh;margin-left: 0.5vw;margin-right: 0.5vw;">{{ name.readCount }}</div>
                        </div>
                        <div v-if="name.readCount>10&&name.readCount<100" style="float: left;margin-top: 0.3vh;font-size: 2.2vh;margin-left: -1.4vw;background-color: red;border-radius: 50%;width: 1.55vw;">
                            <div style="font-size: 1.5vh;color: white;
                            margin-top: 0.45vh;margin-bottom: 0.45vh;margin-left: 0.5vw;margin-right: 0.5vw;">{{ name.readCount }}</div>
                        </div>
                        <div v-if="name.readCount>100&&name.readCount<1000" style="float: left;margin-top: 0.3vh;font-size: 2.2vh;margin-left: -1.4vw;background-color: red;border-radius: 50%;width: 1.7vw;">
                            <div style="font-size: 1.5vh;color: white;
                            margin-top: 0.5vh;margin-bottom: 0.5vh;margin-left: 0.2vw;margin-right: 0.2vw;">{{ name.readCount }}</div>
                        </div>
        ...
        <div v-loading="listLoading" style="float: left;width: 78%;background-color: rgb(250,250,249);height: 96vh;margin-top: 2vh;">
          ...
          text-align: center;background-color: #f0f0f0;border-radius: 1.3vh;">
            ...
            <div v-if="name" style="clear: left;bottom: 10px;width: 99%;height: 5vh;background-color: white">
    ...
    methods:{
        ...
        setMessage(message){
            ...
            if(senderId==this.receiverId){
                ...
                const idd = sessionStorage.getItem("user_id")
                this.$http.get('/api/cloud-message/privateMessage/updateMessage/'+this.receiverId+"/"+idd).then(response => {
                    
                })
            }else{
                for(var i = 0;i<this.userList.length;i++){
                    if(this.userList[i].id==senderId){
                        this.userList[i].readCount = this.userList[i].readCount+1
                        this.boolean = true
                    }
                    if(this.userList[i].id ==senderId){
                    }
                ...
                if(!this.boolean){
                    this.userList.push({id: senderId,nickname: name,readCount: 1,username: name})
                }
                this.boolean = false
                this.$notify.success({
                    title:`${name}发消息来了`,
        ...
        getStyMessage(name){
            ...
            this.$http.get('/api/cloud-message/privateMessage/updateMessage/'+name.id+"/"+id).then(response => {
                for(var i = 0;i<this.userList.length;i++){
                    if(this.userList[i].id==name.id){
                        this.userList[i].readCount = 0
                    }
                }
            })
            this.getList(name)
        ...
        getMessage2(){
            ...
            const startTime = performance.now();
                ...
                for(var i = 0;i<this.userList.length;i++){
                    if(this.userList[i].id==this.receiverId){
                        this.userList[i].readCount = 0
                    }
                }
                const idd = sessionStorage.getItem("user_id")
        ...
        async getList(message){
            this.listLoading = true;
                ...
                if(message!=null){
                    console.log(message)
                    const id = sessionStorage.getItem("user_id")
                    this.$http.get('/api/cloud-message/privateMessage/updateMessage/'+message.id+"/"+id).then(response => {
                        for(var i = 0;i<this.userList.length;i++){
                            if(this.userList[i].id==message.id){
                                this.userList[i].readCount = 0
                            }
                        }
                    })
                    ...
                    this.name = message.nickname!=null?message.nickname:message.username;
